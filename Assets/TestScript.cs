...
namespace kochkaev.TestScript
{
    ...
    //[SerializeField]
    //private zero m_MovementType = zero.Elastic;
    //public zero movementType { get { return m_MovementType; } set { m_MovementType = value; } }
    ...
    public class eventOne
    {
        ...
        public class ArgumentCache : ISerializationCallbackReceiver
        {
            ...
            [SerializeField] private UnityObject m_ObjectArgument;
            ...
            public UnityObject unityObjectArgument
            {
        ...
        public abstract class BaseInvokableCall
        {
            ...
            public static void ThrowOnInvalidArg<T>(object arg)
            {
            ...
            protected static bool AllowInvoke(Delegate @delegate)
            {
                ...
                // UnityEngine object
                var unityObj = target as UnityObject;
        ...
        public class CachedInvokableCall<T> : InvokableCall<T>
        {
            ...
            public CachedInvokableCall(UnityObject target, MethodInfo theFunction, T argument)
                : base(target, theFunction)
            {
                m_Arg1 = argument;
            }
        ...
        public class PersistentCall : ISerializationCallbackReceiver
        {
            ...
            public static BaseInvokableCall GetObjectCall(UnityObject target, MethodInfo method, ArgumentCache arguments)
            {
                var type = typeof(UnityObject);
                if (!string.IsNullOrEmpty(arguments.unityObjectArgumentAssemblyTypeName))
                    type = Type.GetType(arguments.unityObjectArgumentAssemblyTypeName, false) ?? typeof(UnityObject);

                var generic = typeof(CachedInvokableCall<>);
                var specific = generic.MakeGenericType(type);
                var ci = specific.GetConstructor(new[] { typeof(UnityObject), typeof(MethodInfo), type });
            ...
            public void RegisterPersistentListener(UnityObject ttarget, Type targetType, string mmethodName)
            {
                m_Target = ttarget;
        ...
        public class PersistentCallGroup
        {
            ...
            public void RegisterEventPersistentListener(int index, UnityObject targetObj, Type targetObjType, string methodName)
            {
            ...
            public void RegisterVoidPersistentListener(int index, UnityObject targetObj, Type targetObjType, string methodName)
            {
                var listener = GetListener(index);
                listener.RegisterPersistentListener(targetObj, targetObjType, methodName);
            ...
            public void RegisterObjectPersistentListener(int index, UnityObject targetObj, Type targetObjType, UnityObject argument, string methodName)
            {
                var listener = GetListener(index);
                listener.RegisterPersistentListener(targetObj, targetObjType, methodName);
            ...
            public void RegisterIntPersistentListener(int index, UnityObject targetObj, Type targetObjType, int argument, string methodName)
            {
                var listener = GetListener(index);
                listener.RegisterPersistentListener(targetObj, targetObjType, methodName);
            ...
            public void RegisterFloatPersistentListener(int index, UnityObject targetObj, Type targetObjType, float argument, string methodName)
            {
                var listener = GetListener(index);
                listener.RegisterPersistentListener(targetObj, targetObjType, methodName);
            ...
            public void RegisterStringPersistentListener(int index, UnityObject targetObj, Type targetObjType, string argument, string methodName)
            {
                var listener = GetListener(index);
                listener.RegisterPersistentListener(targetObj, targetObjType, methodName);
            ...
            public void RegisterBoolPersistentListener(int index, UnityObject targetObj, Type targetObjType, bool argument, string methodName)
            {
                var listener = GetListener(index);
                listener.RegisterPersistentListener(targetObj, targetObjType, methodName);
            ...
            public void RemoveListeners(UnityObject target, string methodName)
            {
        ...
        public abstract class UnityEventBase : ISerializationCallbackReceiver
        {
            ...
            internal MethodInfo FindMethod(PersistentCall call)
            {
                ...
                if (!string.IsNullOrEmpty(call.arguments.unityObjectArgumentAssemblyTypeName))
                    type = Type.GetType(call.arguments.unityObjectArgumentAssemblyTypeName, false) ?? typeof(UnityObject);
            ...
            internal MethodInfo FindMethod(string name, Type listenerType, PersistentListenerMode mode, Type argumentType)
            {
                switch (mode)
                {
                    ...
                    case PersistentListenerMode.Object:
                        return GetValidMethodInfo(listenerType, name, new[] { argumentType ?? typeof(UnityObject) });
                    default:
                        return null;
                }
            }
            ...
            public UnityObject GetPersistentTarget(int index)
            {
            ...
            {
                return ValidateRegistration(method, targetObj, mode, typeof(UnityObject));
            }
            ...
            protected bool ValidateRegistration(MethodInfo method, object targetObj, PersistentListenerMode mode, Type argumentType)
            {
                ...
                if (!method.IsStatic)
                {
                    var obj = targetObj as UnityObject;
            ...
            protected void RegisterPersistentListener(int index, object targetObj, Type targetObjType, MethodInfo method)
            {
                ...
                m_PersistentCalls.RegisterEventPersistentListener(index, targetObj as UnityObject, targetObjType, method.Name);
                DirtyPersistentCalls();
            }

            internal void RemovePersistentListener(UnityObject target, MethodInfo method)
            {
                if (method == null)
                    return;
            ...
            internal void RegisterVoidPersistentListener(int index, UnityAction call)
            {
                ...
                m_PersistentCalls.RegisterVoidPersistentListener(index, call.Target as UnityObject, call.Method.DeclaringType, call.Method.Name);
                DirtyPersistentCalls();
            }

            internal void RegisterVoidPersistentListenerWithoutValidation(int index, UnityObject target, string methodName)
            {
                RegisterVoidPersistentListenerWithoutValidation(index, target, target.GetType(), methodName);
            }

            internal void RegisterVoidPersistentListenerWithoutValidation(int index, UnityObject target, Type targetType, string methodName)
            {
            ...
            internal void RegisterIntPersistentListener(int index, UnityAction<int> call, int argument)
            {
                ...
                m_PersistentCalls.RegisterIntPersistentListener(index, call.Target as UnityObject, call.Method.DeclaringType, argument, call.Method.Name);
                DirtyPersistentCalls();
            }
            ...
            internal void RegisterFloatPersistentListener(int index, UnityAction<float> call, float argument)
            {
                ...
                m_PersistentCalls.RegisterFloatPersistentListener(index, call.Target as UnityObject, call.Method.DeclaringType, argument, call.Method.Name);
                DirtyPersistentCalls();
            }
            ...
            internal void RegisterBoolPersistentListener(int index, UnityAction<bool> call, bool argument)
            {
                ...
                m_PersistentCalls.RegisterBoolPersistentListener(index, call.Target as UnityObject, call.Method.DeclaringType, argument, call.Method.Name);
                DirtyPersistentCalls();
            }
            ...
            internal void RegisterStringPersistentListener(int index, UnityAction<string> call, string argument)
            {
                ...
                m_PersistentCalls.RegisterStringPersistentListener(index, call.Target as UnityObject, call.Method.DeclaringType, argument, call.Method.Name);
                DirtyPersistentCalls();
            }

            internal void AddObjectPersistentListener<T>(UnityAction<T> call, T argument) where T : UnityObject
            {
                var count = GetPersistentEventCount();
                AddPersistentListener();
            ...
            internal void RegisterObjectPersistentListener<T>(int index, UnityAction<T> call, T argument) where T : UnityObject
            {
                if (call == null)
                    throw new ArgumentNullException("call", "Registering a Listener requires a non null call");

                if (!ValidateRegistration(call.Method, call.Target, PersistentListenerMode.Object, argument == null ? typeof(UnityObject) : argument.GetType()))
                    return;

                m_PersistentCalls.RegisterObjectPersistentListener(index, call.Target as UnityObject, call.Method.DeclaringType, argument, call.Method.Name);
                DirtyPersistentCalls();
            }
